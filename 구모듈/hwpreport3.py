...
def main_page():
    hwp.Open("C:\\Users\\user\\Desktop\\statevl\\hwp\\test1.hwp")
    ...
    hwp.HAction.GetDefault("InsertText", hwp.HParameterSet.HInsertText.HSet)
    hwp.HParameterSet.HInsertText.Text = " 대덕터널 상태평가 보고서"
    hwp.HAction.Execute("InsertText", hwp.HParameterSet.HInsertText.HSet)
...
def report(bujea):  
    ...
    for idx, data in enumerate(xlsx_files_list):
        ...
        클립보드로_이미지_삽입(os.path.join(os.getcwd()+"/table/", img_name))
        hwp.Run("TableColBegin")
        hwp.Run("TableLowerCell")   
        hwp.Run("TableLowerCell")
        hwp.Run("TableLowerCell")
...
{config['Cover']['Project_completiondate']}년에 준공되었다. 댐의 제원 및 시설물의 현황은 다음과 같다.")
    ...
    ##사진 표
    crateTable(1,1)
    클립보드로_이미지_삽입("C:\\Users\\user\\Desktop\\statevl\\StateEstimator\\그림\\그림2_2.png")
    hwp.HAction.Run("MoveDown")
    hwp.HAction.Run("ParagraphShapeAlignCenter")
    ...
    ##사진 표
    crateTable(1,1)   
    클립보드로_이미지_삽입(("C:\\Users\\user\\Desktop\\statevl\\StateEstimator\\그림\\그림2_3.png"))
    hwp.HAction.Run("MoveDown")
    hwp.HAction.Run("ParagraphShapeAlignCenter")
...
def 상태평가기준():
    ...
    글자속성()
    hwp.HAction.Run("BreakPara")
    insert_text(f"본 과업대상 시설물인{config['Cover']['facility_name']}은{config['Cover']['Project_FacilityType']}형식으로{config['Cover']['Project_FacilityType']}에 준하는 기준을 적용하였다.")
    hwp.HAction.Run("BreakPara")
    ...
    ##사진필요
    crateTable(1,1)
    클립보드로_이미지_삽입("C:\\Users\\user\\Desktop\\statevl\\StateEstimator\\그림\\표3_11.png")
    hwp.HAction.Run("MoveDown")
    hwp.HAction.Run("BreakPara")
    글자속성(15,1)
...
if __name__ == '__main__':
    ...
    with open('C:/Users/user/Desktop/statevl/SYD/27SYD_YSR.conf', 'r', encoding='utf-8') as configfile:
...
def makeHwp(damName,bujeaName):
    with open(f'C:/Users/user/Desktop/statevl/{damName}_{bujeaName}.conf', 'r', encoding='utf-8') as configfile:
        config.read_file(configfile)
    ...
    # set save filename
    hwp.HParameterSet.HFileOpenSave.filename = f'C:\\Users\\user\\Desktop\\statevl\\{damName}_{bujeaName}.pdf'
    ...
    hwp.HAction.GetDefault("FileSaveAs_S", hwp.HParameterSet.HFileOpenSave.HSet)
    # set save filename
    hwp.HParameterSet.HFileOpenSave.filename = f'C:\\Users\\user\\Desktop\\statevl\\{damName}_{bujeaName}_H.hwp'
    # set save format to "pdf"
