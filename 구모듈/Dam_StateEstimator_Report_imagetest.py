...
imagepth = os.path.join('test1123', 'test')
print(imagepth)

def insert_image_in_cell(cell, image_path):
    # 이미지를 삽입하기 위해 셀 내에 새로운 Paragraph를 추가합니다.
...
def get_image_files_in_folder(folder_path):
    image_extensions = ['.jpg', '.jpeg', '.png', '.gif', '.bmp']  # 이미지 확장자들
...
def Dam_StateEstimator_Report(config, file, outputpath, saveFile2, reportname):
    print(saveFile2)
    print('상태평가보고서 작성')

    data = pd.read_csv(file)

    image_files = get_image_files_in_folder(imagepth)

    reportroot = 'StateEstimatorReport'
    reportform = os.path.join(reportroot, 'form.docx')
    reportjson = config

    reportpicturepath = os.path.join(reportroot, '그림')
    ...
    # config1.read('.\\DCDStateEstimateSetup.conf', encoding='utf-8')
    configs = os.path.join(reportroot, reportjson)
    ...
    document.add_paragraph('1. 시설물 개요\n', style='SubTitle')
    ...
    document.add_paragraph('2. 상태평가 개요', style='SubTitle')
    ...
    document.add_paragraph('3.2.3 기계 및 전기설비', style='subTitle2')

    document.add_paragraph('4. 상태평가 결과', style='SubTitle')


    document.add_page_break()
    document.add_paragraph('대청다목적댐 제5회 정밀안전진단 결과표', style='TitleName')
    ...
    document.add_page_break()
    document.add_paragraph('대청다목적댐 현황표', style='TitleName')
    ...
    SpillwayDesignFloodLever = '여수로계획홍수량/계획방류량'
    Spillwaygate = config1['여수로게이트']
    Spillwaytype = config1['여수로형식']

    SubSpillwayDesignFloodLever = '보조여수로계획홍수량/계획방류량'
    ...
    hdr_cells[0].text = '여수로'
    hdr_cells[1].text = '여수로계획홍수량/계획방류량'
    hdr_cells[2].text = SpillwayDesignFloodLever
    hdr_cells[3].text = '보조여수로'
    hdr_cells[4].text = '보조여수로계획홍수량/계획방류량'
    ...
    document.add_paragraph("시설물의 상태평가는 「시설물의 안전 및 유지관리 실시 지침(국토교통부, 시설안전공단)」에 따라 실시하며, 상태평가에 대한 세부적인 사항은"
                           " 「시설물의 안전 및 유지관리 실시 세부지침(안전점검·진단편의 댐편, 2019.09)」을 준용하였다. "
    ...
    document.add_paragraph("본 과업대상 시설물인 [보현산 다목적댐]은 [중력식댐(C.G.D)]형식으로 [콘크리트댐]에 준하는 기준을 적용하였다.\n"
    ...
    with open(os.path.join(outputpath, 'merged_file.csv'), "r", encoding="cp949", errors="replace",
        ...
        if data:
            ...
            table.cell(2, 0).merge(table.cell(12, 0))
            # table.cell(2, 0).merge(table.cell(6, 0))
            # table.cell(7, 0).merge(table.cell(12, 0))

            table_cell = table.cell(3, 0)
            insert_image_in_cell(table_cell, image_files[140])


            # table.cell(13, 0).merge(table.cell(23, 0))
            # table.cell(24, 0).merge(table.cell(34, 0))

            table.cell(2, 1).merge(table.cell(7, 1))
            # table.cell(13, 1).merge(table.cell(18, 1))
            # table.cell(24, 1).merge(table.cell(29, 1))
    ...
    document.save(str(outputpath) + '\\'+reportname+'.docx')
    file = open(str(outputpath) + '\\'+reportname+'.pdf', "w")
    file.close()
    convert(str(outputpath) + '\\'+reportname+'.docx', str(outputpath) + '\\'+reportname+'.pdf')
...
if __name__ == '__main__':
    ...
    get_image_files_in_folder(imagepth)
