...
from csvtest_240229 import damlist, namelist, csvwrite
...
def jsonread(json):
    ...
    try:
        with open(json, "r") as json:
            tabledata = json.load(json)
    except IOError as e:
        print('Error occured:', str(e))

    dam_number = str(tabledata["dam_number"])
    dam_location = str(tabledata["dam_location"])
    dam_manager = str(tabledata["dam_manager"])
    dam_phone = str(tabledata["dam_phone"])

    print(dam_number, dam_location, dam_manager, dam_phone)

    return dam_number, dam_location, dam_manager, dam_phone
...
def csvwrite(row_index):
    ...
    # 특정 행의 인덱스 (0부터 시작)
    target_row_index = row_index  # 예를 들어, 세 번째 행을 읽고 싶다면 인덱스는 2가 됩니다.
...
def Dam_StateEstimator_Report(csvlist, file, outputpath, stateestimatorfiles, saveFile2, resultname):
    print(saveFile2)
    print('상태평가보고서 작성')
    ...
    # config1.read(configs, encoding='utf-8')
    ...
    hdr_cells[0].text = '용역명'
    hdr_cells[1].text = '(주)OOO'
    hdr_cells[4].text = '대표자'
    hdr_cells[5].text = 'OOO'
    ...
    hdr_cells[1].text = dam_name
    hdr_cells[3].text = '시설물번호'
    hdr_cells[4].text = dam_number
    ...
    hdr_cells[2].text = dam_river
    hdr_cells[3].text = '저수지제원'
    hdr_cells[4].text = '총저수량'
    hdr_cells[5].text = dam_reservoirvolume

    hdr_cells = table.rows[5].cells
    hdr_cells[1].text = '댐형식'
    hdr_cells[2].text = dam_type
    hdr_cells[4].text = '유효저수량'
    hdr_cells[5].text = dam_realReservoirvolume

    hdr_cells = table.rows[6].cells
    hdr_cells[1].text = '댐정상 표고'
    hdr_cells[2].text = dam_peak
    hdr_cells[4].text = '유역면적'
    hdr_cells[5].text = dam_reservoirarea

    hdr_cells = table.rows[7].cells
    hdr_cells[1].text = '댐 높이'
    hdr_cells[2].text = dam_height
    hdr_cells[4].text = '계획 홍수위'
    hdr_cells[5].text = dam_designfloodleve

    hdr_cells = table.rows[8].cells
    hdr_cells[1].text = '댐 길이'
    hdr_cells[2].text = dam_length
    hdr_cells[4].text = '상시 홍수위'
    hdr_cells[5].text = dam_normalhighwaterlever

    hdr_cells = table.rows[9].cells
    hdr_cells[1].text = '댐 체적'
    hdr_cells[2].text = dam_volume
    hdr_cells[4].text = '저수위'
    hdr_cells[5].text = dam_lowwaterlevel
    ...
    hdr_cells[2].text = dam_designfloodleve
    ...
    document.add_paragraph(dam_name + '댐은 ' + 'OOO' + '에 위치하고, '
                                                      '높이 ' + dam_height + 'm, 길이 ' + dam_length + 'm의 ' + dam_type + '형식으로 ' +
                           dam_startday + '년에 준공되었다. 댐의 제원 및 시설물의 현황은 다음과 같다.')
    ...
    document.add_paragraph('\n[그림2.1]' + dam_name + ' 종평면도', style='tableName')
    ...
    document.add_paragraph('\n[그림2.3]' + dam_name + ' 표준 단면도', style='tableName')
    ...
    document.add_paragraph(
        "본 과업대상 시설물인" + "[" + dam_name + "]은" + "[" + dam_type + "]형식으로" + "[" + dam_type + "]에 준하는 기준을 적용하였다.\n"
    ...
    document.add_paragraph(
        "댐 시설물에 대한 상태평가는 [그림 3.1]과 같이 단계별로 구분할 때 댐 시설물은 통합시설물 (6단계) 에 해당하는 시설물로서 간주하고, 하위단계인 복합시설, 개별시설, 복합부재, 개별부재로 구분한다."
    ...
    FacilityName = stage4data["StructureType"]
...
def listtest(csvdata):
    print(csvdata[0])


if __name__ == '__main__':
    row_index = namelist('사연댐')

    json = 'D:/우림/사내보고서/2023년 작업/SaaS/workspace/01. Dam/Dam_Fill_231204/tabledata.json'
    ...
    file = '../measure/27SYD-5YSR.csv'
    outputpath = '240325'
    stateestimatorfiles = 'J:/00.User/wlchoe/수자원연구원/StateEstimator'
    saveFile2 = os.path.join(outputpath, '4단계결과_' + '.csv')

    Dam_StateEstimator_Report(csvdata, file, outputpath, stateestimatorfiles, saveFile2, '사연댐샘플_240306')
